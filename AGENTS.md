...
- All windows use `decorations: false` — custom title bars. The main window is `transparent: true` + `shadow: false`; there is a single persistent danmaku window with fixed label `danmaku`, created at runtime in `commands/room.rs` with `.transparent(true)` for opacity support. It is never destroyed on room switch.
- Fast room switching: `switch_room` (used by tray room menu and the danmaku-window RoomSwitcher popover) switches rooms in place inside the persistent window — it saves `currentRoomId`, refreshes the tray, closes the old room's drawer windows, updates the window title, and emits `room-switched`; the danmaku window listens for that event and navigates to `/danmaku/{room_id}` in place. Visibility is preserved (hidden stays hidden; visible stays visible without stealing focus). `open_danmaku_window` (RoomPage's enter-room path) shares the bookkeeping but additionally shows + focuses the window and hides the main window.
- Both the tray room submenu and the RoomSwitcher popover list only currently-live rooms (no status dot). Live status is cached in `AppState.live_status` by a 60s poll (`refresh_live_status_cache`, spawned in lib.rs setup); until the first successful fetch the tray falls back to listing all saved rooms. The popover fetches fresh status on open and shows a loading/empty hint. The status map is keyed by **UID** (from `get_status_info_by_uids`), not room id — always match via `room.uid` / `r.uid`.
- Danmaku window close hides to tray (not disconnect). Separate exit button for actual disconnect.
- Tray single-click toggles the current window's visibility only (never swaps windows): the current window is the danmaku window while a room is connected (the `danmaku` window exists, even if hidden), otherwise the main window. Danmaku and main windows are mutually exclusive — entering a room hides the main window, and `exit_room` (a real disconnect, via the danmaku window's exit button) immediately restores it; hiding the danmaku window to tray is display-only, not a disconnect. The tray left-click handler fires only on `button_state == Up` (a physical click emits both Down and Up `Click` events). There is no tray menu item for switching/summoning windows.
