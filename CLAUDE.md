...
- The danmaku window and the main window are mutually exclusive (hard, not a setting): entering a room hides the main window; exiting a room (`exit_room`, a real disconnect) immediately shows the main window again. Hiding the danmaku window (close-to-tray) is a display-only state — the room is still connected, so the danmaku window is still the "current window".
- Tray single-click toggles the *current* window's visibility only (it never swaps between windows): the current window is the danmaku window while a room is connected (a `danmaku-*` window exists, even if hidden), otherwise the main window. There is no tray menu item for switching/summoning windows — switching back to the main window happens via the danmaku window's exit button (`exit_room`), and switching to the danmaku window happens by entering a room from the main window. The tray left-click handler fires only on `button_state == Up` (a physical click emits both Down and Up `Click` events; matching both would toggle twice and look like a no-op flicker).
